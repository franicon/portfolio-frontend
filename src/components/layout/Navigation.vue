  ...
  <nav class="relative mb-32">
    <div class="fixed bg-zinc-900 bg-opacity-60 backdrop-blur border border-zinc-500 left-0 right-0 top-0 hidden md:flex-row md:flex z-30 p-3 max-w-4xl  my-4 mx-auto rounded-full px-6">
      <div class="flex justify-between items-center w-full">
        ...
        <div class="text-white font-light text-[13px]">
          <RouterLink :class="link.active ? 'text-zinc-300' : 'bg-transparent px-3 hover:text-zinc-200'" @click="toggleActive(link)" v-for="link in routeLinks" :key="link.name" :to="link.route">{{ link.name }}
          </RouterLink>
        </div>
      </div>
    </div>
